...
/*
#define CWR_FLAG 0x01
#define ECE_FLAG (CWR_FLAG * 2)
#define URG_FLAG (ECE_FLAG * 2)
#define ACK_FLAG (URG_FLAG * 2)
#define PSH_FLAG (ACK_FLAG * 2)
#define RST_FLAG (PSH_FLAG * 2)
#define SYN_FLAG (RST_FLAG * 2)
#define FIN_FLAG (SYN_FLAG * 2)
*/

#define FIN_FLAG 0x01
#define SYN_FLAG (FIN_FLAG * 2)
#define RST_FLAG (SYN_FLAG * 2)
#define PSH_FLAG (RST_FLAG * 2)
#define ACK_FLAG (PSH_FLAG * 2)
#define URG_FLAG (ACK_FLAG * 2)
#define ECE_FLAG (URG_FLAG * 2)
#define CWR_FLAG (ECE_FLAG * 2)
...
struct tcphdr {
  ...
  unsigned char offset_and_reserved;
  unsigned char flags;
  ...
  while (fread(&packet, sizeof(struct pcappkt) - sizeof(char*) , 1, in) == 1) {
    ...
    if (ntohs(ethernet->type) == 0x0800) {
      ...
      if (ip->type == 6) {
        // printf("TCP PACKET!\n");
        struct tcphdr tcp;
        memcpy(&tcp, IP_PAYLOAD_OFFSET, sizeof(struct tcphdr));

        printf("TCP: srcport=%hu destport=%hu\n", ntohs(tcp.srcport), ntohs(tcp.destport));
        printf("seq=%u ack=%u window=%hu urgent=%hu\n", ntohl(tcp.seq_num), ntohl(tcp.ack_num),
            ntohs(tcp.window), ntohs(tcp.urgent_pointer));
        printf("Flags: ");
        print_tcp_flags(tcp);
        printf("\n");

        // size of the header in bytes;
        unsigned int data_offset = ((tcp.offset_and_reserved & 0xF0) >> 4) * 4;
        unsigned int payloadlen = (ntohs(ip->total_length) - sizeof(struct iphdr) - data_offset);
        if (iplen == 5) { // if no ip options
          payloadlen += sizeof(int);
        }

        // printf("payloadlen = %d\n", payloadlen);
        printf("start of payload: ");
        payloadlen = payloadlen < 80 ? payloadlen : 80;
        if (payloadlen == 0) {
          printf("No data\n");
        }
        else {
          unsigned char * payload = IP_PAYLOAD_OFFSET + data_offset;
          for (int j = 0; j < payloadlen; j++) {
            print_printable(payload[j]);
            printf(" ");
            if (j % 16 == 0 && j != 0) {
              printf("\n");
            }
