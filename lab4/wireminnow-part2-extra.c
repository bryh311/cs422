...
int main(int argc, char** argv) {
  ...
  while (fread(&packet, sizeof(struct pcappkt) - sizeof(char*) , 1, in) == 1) {
    ...
    // parse the time
    time_t time = (time_t)(packet.timestamp);
    char month[4];
    struct tm *time_info;
    int day;
    int year;
    ...
    if (time_info != NULL) {
      ...
      printf("%s %d, %d %02d:%02d:%d ", month, day, year,
          hour, minute, seconds);
    }
    else {
      printf("bad time stamp!");
    }

    if (!is_nano) {
      packet.subtimestamp *= 1000;
    }

    printf("subtimestamp=%uns actual length=%d captured length=%d\n",
        packet.subtimestamp, packet.original_len, packet.captured_len);
    i++;
    free(packet.data);
    packet.data = NULL;
  }
